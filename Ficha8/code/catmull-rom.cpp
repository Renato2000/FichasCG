...
void getCatmullRomPoint(float t, float *p0, float *p1, float *p2, float *p3, float *pos, float *deriv) {
	...
	for(int i = 0; i < 2; i++){
		...
		multMatrixVector(maux, p, res);

		// Compute pos = T * A 
		pos[i] = pow(t,3) * res[0] + pow(t,2) * res[1] + t * res[2] + res[3];

		// compute deriv = T' * A
		deriv[i] = 3 * pow(t,2) * res[0] + 2 * t * res[1] + res[2];
...
void renderScene(void) {
	...
	glutSwapBuffers();
	t += 0.0001;
	// Para manter a velocidade constante: t += 0.0001 / len;
}
